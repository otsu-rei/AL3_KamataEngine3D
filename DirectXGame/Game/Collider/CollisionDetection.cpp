...
bool CollisionDetection::SphereTo(
	...
	float distance = Vector::Length(positionA - positionB);
...
bool CollisionDetection::SphereToAABB(
	...
	Vector3f closestPoint = Vector::Clamp(posisionA, aabbB.localMin + posisionB, aabbB.localMax + posisionB);

	float distance = Vector::Length(closestPoint - posisionA);
