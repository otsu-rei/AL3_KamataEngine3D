...
void ColliderManager::DrawSphereCollider(const Vector3f& position, const CollisionBoundings::Sphere& sphere) const {
	...
	for (uint32_t lon = 0; lon < kSubdivision; ++lon) {
		...
		drawer->DrawLine3d(start + position, end + position, color_);
	}
...
void ColliderManager::DrawAABBCollider(const Vector3f& position, const CollisionBoundings::AABB& aabb) const {
	...
	for (int i = 0; i < 4; ++i) {
		int next = (i + 1) % 4;
		drawer->DrawLine3d(pos[i] + position, pos[next] + position, color_);
		drawer->DrawLine3d(pos[i + 4] + position, pos[next + 4] + position, color_);
		drawer->DrawLine3d(pos[i] + position, pos[i + 4] + position, color_);
	}
}
